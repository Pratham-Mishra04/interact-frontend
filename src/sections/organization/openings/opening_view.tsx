...
interface Props {
  ...
  opening: Opening;
  openings: Opening[];
  setOpenings: React.Dispatch<React.SetStateAction<any[]>>;
...
const ViewOpening = (props: Props) => {
  ...
  const orgId = props.opening.organizationID;

  const handleButtonClick = async (e: any) => {
    const type = e.target.dataset.type;
    const URL = `org/${orgId}/orgopenings/${props.opening.id}`;
    ...
    if (res.statusCode === 204) {
      ...
      setDeletedOpeningId(props.opening.id);
      props.setClickedOpening(initialOpening);
      props.setOpenings(prev =>
        prev.map(o => {
          if (o.id != deletedOpeningId) {
            return o;
          }
        })
      );
  ...
  return (
    <div className="bg-[#fff] w-screen h-screen absolute z-[100]  text-[#000] gap-[1rem] flex flex-row justify-between items-center p-[2rem]  transition-all duration-200 ">
      <Cross
        ...
        weight="regular"
        onClick={() => {
          props.setClickedOnOpening(false);
        }}
        color="black "
      />
      <Image
      ...
      <div className="flex flex-col justify-around items-start gap-[1rem] w-[25%] h-screen">
        <div className="font-semibold text-lg">
          ...
          <div className="flex flex-row flex-wrap w-full gap-[1rem] font-regular ">
            Tags:
            {props.opening.tags.map((tag, index) => {
              return (
                <div key={index} className=" border-[2px] border-[#af7676] rounded-[0.2rem] p-[2px]">
                  {tag}
                </div>
              );
            })}
        ...
        <div className="w-[100%] flex flex-col justify-center gap-[1rem]">
          ...
          <div>
            <div className="text-xs ml-1 font-medium uppercase">Opening Tags ({tags.length || 0}/10)</div>
            <Tags tags={tags} setTags={setTags} maxTags={10} />
          </div>
          <div className="flex flex-row-reverse justify-end items-center">
            <div className="text-xs ml-1 font-medium uppercase">Opening Activity Status</div>
